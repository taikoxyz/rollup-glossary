...
## Bytecode-equivalent and EVM-equivalent ZK-EVM
...
## Bytecode-compatible and EVM-compatible ZK-EVM
...
A type of layer 2 scaling solution that batches multiple transactions and submits them to a separate chain (like an L1) in a single transaction. The validity of the batch and the transactions within is proven either up front with a validity proof (ZK-Rollup), or after some challenge period has elapsed without a fraud proof being submitted (Optimistic Rollup). Rollups allow for reductions in gas costs and increases in transaction throughput while inheriting security guarantees from the L1/DA layer to which they posted the transaction data.
